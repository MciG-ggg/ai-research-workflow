...
def registry_row(main_worktree: Path, worktree: Path, branch: str) -> str | None:
    registry = main_worktree / ".omx" / "worktrees" / "REGISTRY.md"
...
def build_plan(worktree_arg: Path, base_arg: str | None, validation_paths: list[str]) -> dict[str, Any]:
    ...
    if registry.exists() and row is None:
        blockers.append(".omx/worktrees/REGISTRY.md exists but no matching row was found")
...
```bash
{commands}
# After successful merge validation, update .omx/worktrees/REGISTRY.md to merged/removed.
```
"""
