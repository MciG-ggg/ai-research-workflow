...
def status(installed_dir: Path, *, check_remote: bool, remote_url: str | None) -> dict[str, Any]:
    ...
    result = {
        ...
        "remote_head": None,
        "update_command": "./skills/ai-research-workflow/scripts/update_installed_skill.sh",
    }
...
def main() -> int:
    default_dest = Path.home() / ".codex" / "skills" / "ai-research-workflow"
    parser = argparse.ArgumentParser(description="Check source/installed/remote ai-research-workflow version status.")
    parser.add_argument("--installed-dir", type=Path, default=default_dest, help="Installed skill directory. Defaults to ~/.codex/skills/ai-research-workflow.")
