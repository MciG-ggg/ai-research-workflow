...
def summarize_workstream(workstream: Path) -> dict[str, Any]:
    ...
    missing = [name for name in CONTROL_FILES if not (workstream / name).is_file()]
...
def markdown(summary: dict[str, Any]) -> str:
    ...
    for item in summary["workstreams"]:
        ...
        rows.append(
            f"| {item['slug']} | {item['phase']} | {item['phase_status']} | {item['run_count']} | {item['script_count']} | {missing} | {blockers} | {item['next_action']} |"
        )
    if not rows:
        rows = ["| none | n/a | n/a | 0 | 0 | n/a | n/a | create a gated workstream |"]
    return f"""# Research State Summary
...
| Slug | Phase | Status | Runs | Scripts | Missing artifacts | Blockers | Next action |
| --- | --- | ---: | ---: | ---: | --- | --- | --- |
{chr(10).join(rows)}
