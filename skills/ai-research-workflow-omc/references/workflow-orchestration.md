...
- Use Paper Registry Scouting when the user asks to find SOTA/baseline papers or maintain a paper list; write `.omc/ai-research/PAPERS.md`.
...
3. Run when `.omc/ai-research/CONFIG.md` sets `idea_scouting: on`.
...
Write `.omc/ai-research/IDEA_SCOUTING.md` using `assets/templates/IDEA_SCOUTING.md`. The scouting pass may search sources, generate candidates, rank/filter them, and mark weak ideas as parked or rejected.

When the user asks to find SOTA/baseline papers, write `.omc/ai-research/PAPERS.md` using `assets/templates/PAPERS.md`. This paper registry is portfolio-level and should be maintained across scouting passes. It tracks paper role, key claim, benchmark/metric, code/data/checkpoint availability, reproduction priority, status, and maintenance history.
...
Promotion gate: recommend a candidate for formal intake only when it has a falsifiable hypothesis, evaluation metric/baseline, lightweight evidence, novelty-risk note, feasibility budget, and user-goal fit. Ask the user before creating `.omc/ai-research/<slug>/`; on confirmation, enter the new-workstream mandatory workflow gate.
...
3. `.omc/ai-research/CONFIG.md` can set `workflow_preset: conservative | guided | autonomous`, `idea_scouting: auto | off | on`, `completion_handoff: auto | off`, `worktree_closeout: off | report-before-merge`, `feedback_memory: off | lite | full`, `qa_capture: off | research | all`, and `growth_review: off | milestone | always`.
...
Every project gets a root portfolio layer under `.omc/ai-research/`:

- `.omc/ai-research/RESEARCH.md`: the overall research program, central question, north-star hypotheses, claim boundaries, current synthesis, and next priorities.
- `.omc/ai-research/INDEX.md`: the workstream registry mapping each `<slug>` to its subquestion, status, artifact links, latest evidence, and next action.
- `.omc/ai-research/PAPERS.md`: optional SOTA/baseline paper registry when paper scouting or paper reproduction is requested.

Each `.omc/ai-research/<slug>/` directory is a workstream, not the whole research program. It should link back to the portfolio `RESEARCH.md` and appear in `INDEX.md`.
...
2. List existing `.omc/ai-research/<slug>/` directories.
...
The gate blocks creating `.omc/ai-research/<slug>/`, implementation, experiment launch, or docs publishing until the portfolio `INDEX.md` records these evidence paths:
...
`.omc/ai-research/` is the portfolio control plane. `.omc/ai-research/<slug>/` is a workstream control plane. Together they store specs, decisions, run indexes, evidence summaries, reproducibility notes, and the global synthesis.
...
Do not implement the user's method, model, baseline, dataset adapter, or production experiment code inside `.omc/ai-research/`. Only thin orchestration wrappers, run metadata, and research notes belong there.
...
1. Update `.omc/ai-research/<slug>/RUNS.md` with command, seed/device/resource, status, log path, metrics path, summary path, and failure notes.
2. Update `.omc/ai-research/<slug>/SCRIPT_REGISTRY.md` with scripts or native commands that were used, changed, validated, deprecated, or requested by the user.
3. Update `.omc/ai-research/<slug>/RESULTS.md` with tables, figures, hypothesis verdicts, and interpretation when the run changes the scientific picture.
4. Update `.omc/ai-research/<slug>/REPRODUCIBILITY.md` with new blockers, environment notes, nondeterminism, or rerun instructions.
5. Promote stable conclusions to project-root `docs/`, `reports/`, benchmark cards, config docs, or README sections when useful.
6. Promote reusable implementation changes to project-root code/config/tests, not to `.omc/ai-research/`.
...
When distillation changes the global picture, update `.omc/ai-research/RESEARCH.md` and `.omc/ai-research/INDEX.md` in the same pass so the portfolio layer remains the user's overall map of the work.

For a paper-reproduction workstream, completion handoff also updates `.omc/ai-research/PAPERS.md` with reproduction status, evidence links, blockers, and revised priority. If the reproduction generated new candidate ideas, update `.omc/ai-research/IDEA_SCOUTING.md` or the relevant workstream artifacts with those ideas before closing the stream.
