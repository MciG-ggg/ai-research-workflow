...
Use these contracts to keep scientific goals separate from execution details and to keep `.omc/ai-research` metadata separate from project-root implementation work.
...
`.omc/ai-research/` has a portfolio control plane and one or more workstream control planes.
...
`.omc/ai-research/RESEARCH.md` stores the overall research program: central question, north-star hypotheses, success criteria, claim boundaries, current synthesis, active subquestions, and next priorities.

`.omc/ai-research/INDEX.md` stores the workstream registry: each `<slug>`, status, relationship to the overall question, artifact links, latest evidence, and next action.

`.omc/ai-research/<slug>/` is the workstream control plane. It stores research intent, plans, run indexes, evidence summaries, and reproducibility notes for one concrete direction.

Each workstream has `STATE.json` for phase state and `CLAIMS.md` for the authoritative evidence-to-claim ledger.

Project-root files are the implementation plane. Method code, baseline or paper reproduction code, configs, tests, dataset adapters, benchmark entrypoints, and durable docs belong in the target repository's normal locations, not under `.omc/ai-research/`.
...
`IDEA_SCOUTING.md` is optional and belongs at the portfolio root. It is used only before a clear research question exists, when the user asks for idea generation, idea triage, broad-direction scouting, `.omc/ai-research/CONFIG.md` sets `idea_scouting: on`, or `idea_scouting: auto` applies under a guided/autonomous preset.
...
Research Feedback Memory, Question Capture, and Researcher Growth Review are opt-in. They are disabled by default and must not create extra files unless enabled by invocation flags or `.omc/ai-research/CONFIG.md`.
...
Optional `.omc/ai-research/CONFIG.md` fields:
...
Minimum `.omc/ai-research/RESEARCH.md` sections:
...
Minimum `.omc/ai-research/INDEX.md` sections:
...
Before creating a new workstream, inspect portfolio `RESEARCH.md`, portfolio `INDEX.md`, and existing `.omc/ai-research/<slug>/` directories. Reuse an existing workstream unless the new task has a distinct research question, validation target, or artifact boundary.

New workstream mandatory workflow gate:

- `/oh-my-claudecode:deep-interview --autoresearch` must produce or link a validator-ready mission/intake artifact before a new slug is opened.
- `/oh-my-claudecode:ralplan` must produce or link consensus planning output, `.omc/plans/prd-<slug>.md`, and `.omc/plans/test-spec-<slug>.md`.
- `/oh-my-claudecode:autoresearch` must persist state with a `completion_artifact_path` and link `.omc/specs/autoresearch-<slug>/mission.md`, `sandbox.md`, and `result.json`.
...
- Project-local wrapper scripts under `.omc/ai-research/<slug>/scripts/`
...
- Promote reusable code/config/test changes to the project root, not to `.omc/ai-research/`.
